...
export default class SignInPage extends BaseComponent {
  ...
  onSubmit(event) {
    ...
    if (!username) {
      errors.username = i18n.__('pages.authPageSignIn.emailRequired');
    }
    if (!password) {
        ...
        <form onSubmit={this.onSubmit}>
          ...
          <div className={`input-symbol ${errorClass('email')}`}>
            <input
              ...
              placeholder={i18n.__('pages.authPageSignIn.yourUsername')}
            />
            <span
              className="icon-edit"
              title={i18n.__('pages.authPageSignIn.yourUsername')}
            />
          </div>
